...
contains
  ...
  ! =====================================================================================
module osmotic_potential_mod
  implicit none
contains
  ! Function to calculate osmotic potential in the soil..to be added to psi_from_th_vg
  function calc_osmotic_potential(salinity, E, R, iv, T) result(osmotic_potential)
    ! Parameters
    real(8), intent(in) :: salinity  ! Salinity in ppt
    real(8), intent(in) :: E          ! Filtration efficiency
    real(8), intent(in) :: R          ! Gas constant [J mol^-1 K^-1]
    real(8), intent(in) :: iv         ! Van't Hoff coefficient
    real(8), intent(in) :: T          ! Temperature [K]
    
    ! Local variable
    real(8) :: C                       ! Solute concentration in mol/m^3
    real(8) :: osmotic_potential        ! Osmotic potential [Pa]

    ! Calculate concentration of NaCl in mol/m^3
    C = salinity * 1000.0 / 58.44

    ! Calculate osmotic potential in Pascals [Pa]
    osmotic_potential = -E * C * R * iv * T
  end function calc_osmotic_potential
end module osmotic_potential_mod
